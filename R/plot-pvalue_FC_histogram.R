...
#' @param columnNameP      \strong{characte} \cr
#'                          The column name for p-value.
#' @param columnNamePadj   \strong{characte} \cr
#'                          The columns name for adjusted p-value.
#' @param columnNameFC     \strong{characte} \cr
...
pvalue_foldchange_histogram <- function(RES,
  ...
  D <- data.frame(p = RES[[columnNameP]],
                  padj = RES[[columnNamePadj]],
                  FC = RES[[columnNameFC]])

  D <- na.omit(D) # remove NAs before plotting
